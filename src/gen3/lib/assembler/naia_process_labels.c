  ...
  (naia_t* naia)
{
  ...
  for (i=0; i < naia->captures->count; i++) {
    ...
    case ASMSLOT_ISOLATEINSTR_ISOLATE:
      offset += NAIG_INSTR_LENGTH(OPCODE_ISOLATE);
      break;
    case ASMSLOT_ENDISOLATEINSTR_ENDISOLATE:
      offset += NAIG_INSTR_LENGTH(OPCODE_ENDISOLATE);
      break;

    case ASMSLOT_SCR_ADD_SADD:
      offset += NAIG_INSTR_LENGTH(OPCODE_SCR_ADD);
      break;
    case ASMSLOT_SCR_CALL_SCALL:
      offset += NAIG_INSTR_LENGTH(OPCODE_SCR_CALL);
      break;
    case ASMSLOT_SCR_PUSH_SPUSH:
      offset += NAIG_INSTR_LENGTH(OPCODE_SCR_PUSH);
      break;
