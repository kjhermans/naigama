...
my $disassembler = "$assembler";
$disassembler =~ s/ass/disass/g;
$disassembler =~ s/naia/naid/;
$disassembler =~ s/ .*$//;
  ...
  if (scalar(@fields) >= 3) {
    ...
    if ($x) {
      ...
      if ($fields[2] eq 'ERR_COMP') {
        print "Test Ok\n";
        system("echo '======== TEST $n' >> /tmp/success.$file.log");
        system("cat $tmpfile.$n.log >> /tmp/success.$file.log");
      } else {
        print "Test NOK\n";
        system("echo '======== TEST $n' >> /tmp/failure.$file.log");
