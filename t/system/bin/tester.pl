...
$SIG{CHLD} = sub {
  while( ( my $child = waitpid( -1, &WNOHANG ) ) > 0 ) {
    ...
    foreach my $nodename (keys(%{$description->{nodes}})) {
      ...
      foreach my $test (@{$node->{tests}}) {
        if ($test->{pid} eq $child) {
          my $exitcode = $?;
          print STDERR "\@$time: PID $test->{pid} $test->{exe} EXIT $exitcode\n";
    ...
    foreach my $test (@{$description->{tests}}) {
      if ($test->{pid} eq $child) {
        my $exitcode = $?;
        print "\@$time: PID $test->{pid} $test->{exe} EXIT $exitcode\n";
        if ($test->{critical} && $exitcode ne '0') {
          $failed = 1;
        }
        if ($test->{pivotal}) {
...
## Tests
  ...
  for (my $i=0; $i < $max && !$stopped && !$failed; $i++) {
    ...
    foreach my $test (@{$start[ $i ]}) {
      my $cmd = $test->{exe};
      while ($cmd =~ s/\[([a-zA-Z0-9_]+)\]/$ENV{$1}/g) { }
      if ($test->{namespace}) {
  ...
  if ($failed) {
    print STDERR "Breaking out of loop because critical process failed\n";
  }
  if ($stopped) {
    print STDERR "Breaking out of loop because pivotal process exited\n";
  }
  my $t = localtime();
