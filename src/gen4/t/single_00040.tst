...
SAi1         <- SA
SAr1         <- SA
SA           <- NextPayload CFlags PayloadLength Proposals

Proposals    <- Proposal+
Proposal     <- LastOrMore RESERVED PropLength PropNum
                ProtoID SPISize NumTransforms
LastOrMore   <- 0x00 / 0x02
RESERVER     <- .
PropLength   <- { .. }
PropNum      <- { . }
ProtoID      <- { . }
