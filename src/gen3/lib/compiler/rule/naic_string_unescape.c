  ...
  )
{
  ...
  while ((c = *string) != 0) {
    ...
    case '\\':
      ...
      case 'r': CHECK(fnc(naic, '\r', last)); break;
      case 't': CHECK(fnc(naic, '\t', last)); break;
      case 'v': CHECK(fnc(naic, '\v', last)); break;
      case '\\': CHECK(fnc(naic, '\\', last)); break;
      case '\'': CHECK(fnc(naic, '\'', last)); break;
      ...
      case 'F':
        ...
        CHECK(fnc(naic, hexcodon(c, string[ 1 ]), (string[ 2 ] == 0)));
        ++string;
      }
      break;
    default:
      CHECK(fnc(naic, c, (string[ 1 ] == 0)));
    }
    ++string;
  }
  return NAIG_OK;
